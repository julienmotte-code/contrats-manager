...
export default function CommandesPlanifiees() {
  ...
  const fetchCommandes = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/api/commandes/planifiees', {
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <EventIcon color="primary" /> Commandes planifiées
        </Typography>
        <Chip label={`${total} commande(s)`} color="primary" />
      </Box>
            ...
            {loading ? (
              ...
              commandes.map((cmd) => (
                <TableRow 
                  key={cmd.id} 
                  hover
                  sx={isDatePassed(cmd.date_planifiee) ? { backgroundColor: 'warning.light' } : {}}
                >
                  <TableCell>
                  ...
                  <TableCell>
                    <Chip 
                      label={formatDate(cmd.date_planifiee)}
                      size="small"
                      color={isDatePassed(cmd.date_planifiee) ? 'warning' : 'default'}
                    />
                  </TableCell>
                  <TableCell>{cmd.intervenant_nom || '-'}</TableCell>
        ...
        <DialogContent>
          {detailCommande && (
            <Box sx={{ mt: 1 }}>
              ...
              <Typography variant="subtitle2" color="text.secondary">Date intervention</Typography>
              <Typography gutterBottom>{formatDate(detailCommande.date_planifiee)}</Typography>
              
              <Typography variant="subtitle2" color="text.secondary">Intervenant</Typography>
              <Typography gutterBottom>{detailCommande.intervenant_nom || '-'}</Typography>
              
              {detailCommande.notes_planification && (
                <>
                  <Typography variant="subtitle2" color="text.secondary">Notes</Typography>
                  <Typography gutterBottom>{detailCommande.notes_planification}</Typography>
                </>
              )}
