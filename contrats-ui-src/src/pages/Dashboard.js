...
function FamilleCard({ famille, clickable = true }) {
  const icon = FAMILLE_ICONS[famille.code] || '📋';
  const colors = FAMILLE_COLORS[famille.code] || FAMILLE_COLORS.AUTRE;

  const content = (
    <div className={`block border rounded-xl p-4 ${clickable ? 'transition-all hover:shadow-md hover:scale-[1.02]' : ''} ${colors}`}>
  ...
  return clickable ? <Link to={`/contrats?famille=${famille.code}`}>{content}</Link> : content;
}

function CommandeStatutCard({ statut, count, clickable = true }) {
  const config = STATUT_CONFIG[statut];
  if (!config) return null;

  const content = (
    <div className={`block border rounded-xl p-4 ${clickable ? 'transition-all hover:shadow-md hover:scale-[1.02]' : ''} ${config.color}`}>
      <div className="flex items-center gap-3">
  ...
  return clickable ? <Link to={config.link}>{content}</Link> : content;
}
...
export default function Dashboard() {
  ...
  const isIntervenant = isFormateur || isTechnicien;

  useEffect(() => {
    const requests = [api.get('/api/dashboard/stats')];
    if (!isIntervenant) {
      requests.push(indicesAPI.courant().catch(() => null));
    ...
    Promise.all(requests).then(([statsRes, indiceRes]) => {
  ...
  const contratsTechnicien = contrats.filter(f => FAMILLES_TECHNICIEN.includes(f.code));
  ...
  return (
    <div className="space-y-6">
      ...
      {isTechnicien && (
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900">📄 Contrats techniques</h2>
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-500">
                {contratsTechnicien.reduce((s, f) => s + (f.total || 0), 0)} contrat(s)
              </span>
              <Link to="/contrats" className="text-blue-600 text-sm hover:underline">Voir tout →</Link>
            </div>
          </div>
          {contratsTechnicien.length === 0 ? (
            <p className="text-gray-400 text-sm text-center py-6">Aucun contrat technique</p>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {contratsTechnicien.map(f => (
                <FamilleCard key={f.code} famille={f} clickable={true} />
              ))}
            </div>
          )}
        </div>
      ...
      <div className="card">
        ...
        <div className={`grid gap-3 ${isIntervenant ? 'grid-cols-2' : 'grid-cols-3'}`}>
          {!isIntervenant && (
            <CommandeStatutCard statut="nouvelles" count={commandes.nouvelles || 0} clickable={true} />
          )}
          <CommandeStatutCard statut="a_planifier" count={commandes.a_planifier || 0} clickable={false} />
          <CommandeStatutCard statut="planifiees" count={commandes.planifiees || 0} clickable={false} />
        </div>
      </div>

      {!isIntervenant && indice?.indice && (
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
