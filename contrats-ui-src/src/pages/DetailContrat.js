...
export default function DetailContrat() {
  ...
  const validerProrata = async () => {
    ...
    try {
      await api.put(`/api/contrats/${id}`, { prorate_validated: true });
      toast.success('Prorata validé');
      setContrat(c => ({ ...c, prorate_validated: true }));
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          ...
          {contrat.statut === 'BROUILLON' && (
            <Link to={`/contrats/${id}/modifier`} className="btn-secondary text-sm">✏️ Modifier</Link>
          )}
          {contrat.statut === 'BROUILLON' && (
            <>
              ...
              {contrat.prorate_annee1 && !contrat.prorate_validated && (
                <button onClick={validerProrata} disabled={prorataValidating} className="btn-secondary text-sm">
                  ✅ Valider le prorata
                </button>
              )}
          ...
          {contrat.statut === 'EN_COURS' && (<><button onClick={() => setShowRenouveler(true)} className="btn-secondary text-sm">🔄 Renouveler</button><button onClick={() => setShowTerminer(true)} className="btn-danger text-sm">🚫 Terminer</button></>)}
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="card space-y-3">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-gray-500">Début</span><span>{contrat.date_debut ? format(new Date(contrat.date_debut), 'dd/MM/yyyy') : '-'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">Fin</span><span>{contrat.date_fin ? format(new Date(contrat.date_fin), 'dd/MM/yyyy') : '-'}</span></div>
            ...
            <tbody className="divide-y divide-gray-100">
              {contrat.plan_facturation.map(p => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">{p.date_echeance ? format(new Date(p.date_echeance), 'dd/MM/yyyy') : '-'}</td>
